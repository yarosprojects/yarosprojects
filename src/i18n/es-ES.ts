export default {
    months: {
        january: 'Enero',
        february: 'Febrero',
        march: 'Marzo',
        april: 'Abril',
        may: 'Mayo',
        june: 'Junio',
        july: 'Julio',
        august: 'Agosto',
        september: 'Septiembre',
        october: 'Octubre',
        november: 'Noviembre',
        december: 'Diciembre'
    },
      

    cities: {
        psip: "Palau-solità i Plegamans",
        barcelona: "Barcelona",
    },

    regions: {
        catalonia: "Cataluña",
    },

    countries: {
        spain: "España",
    },

    creator_name: "Yaroslav Klymko",
    all_rights_reserved: 'Todos los derechos reservados',
    work_experience: 'Experiencia laboral',
    section_in_mantainance: "Ésta sección se encuentras en tareas de mantenimiendo y mejora. Disuclpe las molestias y gracias por su paciencia",

    themes: {
        light: 'Claro',
        dark: 'Oscuro',
        system: 'Sistema',
    },

    nav: {
        intro: 'Intro',
        experience: 'Experiencia',
        projects: 'Proyectos',
        about_me: 'Sobre mí',
        contact: 'Contacto',
    },

    intro: {
        show_more: "Mostrar más",
        show_less: 'Mostrar menos',
        hello_iam: "Hola, soy",
        fullstack_web_developer: "Desarrollador web full stack",
        years_of_experience: "años de experiencia",
        work_plus_personal_experience: "Laboral + Personal",
        developer: "Desarollador",
        web_applications: "las aplicaciones web",
        bioinformatical_profile: "perfil",
        bioinformatical: "bioinformático",
        from: "de",
        focused_on: "Enfocado en la",
        creation: "creación",
        maintenance: "mantenimiento",
        resolution: "resolución",
        and: "y la",
        vulnerability: "vulnerabilidades",
        of_static_and_dynamic_web_pages: "de las páginas webs",
        and_normal: "y",
        of_normal: "",
        of_the: "de las",
        statics: "estáticas",
        dynamics: "dinámicas",
        websites: "",
    },  

    trajectory: {
        title: 'Trayectoria',
        actually: 'Actualidad...',
        intership_message: 'Contrato de práticas',
        experiences: {
            'angular': {
                role: 'Desarollador Frontend de Angular',
                description: "Desarrollo de interfaces web con Angular y optimización de rendimiento en la oficina de la cartografia de la Diputación de Barcelona.",
            },
            'peon': {
                role: "Peón",
                description: "Mantenimiento y limpieza de maquinas de producción.",
            },
            'peon-administrativo': {
                role: "Peón - Administrativo",
                description: "Mantenimiento y limpieza de maquinas de producción - Registrando facturas, realizando albaranes y otros tipos de tareas de la ofimática.",
            },
        },
    },

    projects: {
        code_btn: 'Código',
        preview: 'Preview',
        calculator: {
            name: 'Calculadora',
            paragraph_1: "Una calculadora que realiza todo tipo de operaciones de '+', '-', '*' y '/'.",
            paragraph_2: "Y como operaciones adicionales: raíces y porcentajes.",
        },
        tetris: {
            name: 'Tetris',
            paragraph_1: "Un juego clásico de Tetris hecho en Svelte.",
            paragraph_2: "Incluye controles móviles y de teclado, puntajes y niveles progresivos.",
        },
    },

    about_me: {
        iam: 'Soy',
        and: 'y',
        and_ua: '',
        graduated_in: 'especialista en',
        web_develpemn_specialist: 'Desarollo web',
        with_experience_left: 'con experiencia en una amplia variedad de lenguajes de programación',
        with_experience_right: 'y otras tecnologías',
        i_like_create: 'Me encanta crear proyectos',
        innovative: 'innovadores',
        projects: '',
        combine: 'que combinan',
        functionality: 'funcionalidad',
        design: 'diseño',
        exploring_to: 'explorando desde aplicaciones web hasta los',
        creative_experiments: 'experimentos creativos',
        every_project_is: 'Cada proyecto es una',
        opportunity: 'oportunidad',
        of_learn: 'de',
        to_learn: 'aprender',
        improve: 'mejorar',
        my_habilities: '',
        share: 'compartir',
        my_passion: 'mi pasión por la',
        programming: 'programación',
        with_community: 'con la comunidad',
    },

    footer: {
        web_developer: "Desarollador web",
        apasionated: "apasionado por las curiosidades de la",
        programming: "programación",
        and: "y",
        web_pages_creation: "creación de páginas webs",
        ensure_quality_for_clients: "asegurar la calidad para los clientes",
    },

    404: {
        error_code: 404,
        error_title: 'Página no encontrada',
        go_back: 'Volver atrás',
        opps: '¡Opps!',
        had_trouble: 'He tenido problemas para encontrar la página solicitada',
        consider_is_error: '¿Crees que se trata de un error?',
        contact_me: 'Contáctame',
    },


    temporary_text: `Soy Yaroslav Klymko, graduado en Desarrollo Web, con experiencia en una
        amplia variedad de lenguajes y tecnologías. Me encanta crear proyectos
        innovadores que combinan funcionalidad y diseño, explorando desde
        aplicaciones web hasta experimentos creativos. Cada proyecto es una
        oportunidad de aprender, mejorar y compartir mi pasión por la
        programación con la comunidad.
    `,

}